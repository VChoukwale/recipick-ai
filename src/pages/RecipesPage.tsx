    ...
    <div className={`rounded-2xl overflow-hidden hover:shadow-md transition-shadow duration-200 ${recipe.rating === -1 ? 'opacity-70' : ''}`} style={{ background: 'var(--s2)', border: recipe.rating === -1 ? '1px solid #dc262630' : '1px solid var(--bdr-s)', boxShadow: 'var(--shd-sm)' }}>
      <div className="px-4 pt-4 pb-3">
        ...
        <div className="flex items-center gap-2 flex-wrap mb-2.5">
          ...
          {recipe.cuisine_type && (
            <span className="text-xs font-body px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400">
          ...
          <div className="flex flex-col items-center justify-center h-48 gap-2 text-center">
            ...
            {hasFilters && (
              <button onClick={() => { setSearch(''); setDifficultyFilter('all'); setCuisineFilter('All'); setFavoritesOnly(false) }}
                className="text-xs text-emerald-600 dark:text-emerald-400 font-display font-600 mt-1">Clear all filters</button>
            )}
          </div>
        ) : (
